...
<ul>
    ...
    <li>מתוך מחשבה על מידע שיכול להיות מסווג, האתר מצבע הצפנה של הססמא בעת ההרשמה בעזרת () ומידור בין משתמשים באמצעות שם משתמש וסיסמא יחודיים לכל אחד.</li>
    <li>בנוסף, בעת ביצוע login התא משייך ליסשיין הנוכחי של היוזר JWT token ושומר אותו בקוקיז, ובעת כניסה לכל דף מודוא שאכן זה היוזר שנכנס מקודם. על מנת לשמור על בטחון המידע של החייל</li>
    <li>לאחר הכנסת הקריטריונים הדרושים, התוכנה מוציאה את רשימת השמירה כפלט של STRING בתצורה מיוחדת לוואטסאפ, על מנת שיהיה ניתן לשתף בקלות בקבוצות בצורה קריאה ואלגנטית הוואטאספ הרלוונטיות.</li>
